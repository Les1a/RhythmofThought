...
class GRPOTrainer(Trainer):
    ...
    def _get_per_token_logps(self, model, input_ids, inputs_embeds, attention_mask, logits_to_keep):
        ...
        logits = model(input_ids=input_ids, inputs_embeds=inputs_embeds, attention_mask=attention_mask, logits_to_keep=logits_to_keep + 1).logits
        ...
        with torch.inference_mode():
            if self.ref_model is not None:
                ...
                with self.accelerator.unwrap_model(self.model).disable_adapter():
                    ref_per_token_logps = self._get_per_token_logps(
                        self.model, prompt_completion_ids, None, attention_mask, logits_to_keep
                    )
