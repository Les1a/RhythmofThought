...
def main(args):
    ...
    if is_grpo:
        exp_name = (f"./experiments/{args.model_name.split('/')[-1]}-rag-grpo-group{args.group_size}"
                    f"-lora{args.lora_rank}-temp{args.temperature}")
    elif is_tgrpo:
        exp_name = (f"./experiments/{args.model_name.split('/')[-1]}-rag-tgrpo-group{args.group_size}"
                    f"-lora{args.lora_rank}-temp{args.temperature}")
    elif is_thrpo:
        exp_name = (f"./experiments/{args.model_name.split('/')[-1]}-rag-thrpo-group{args.group_size}"
                    f"-lora{args.lora_rank}-rmin{args.residual_r_min}-temp{args.temperature}")
    else:
        exp_name = (f"./experiments/{args.model_name.split('/')[-1]}-rag-hrpo-group{args.group_size}"
                    f"-lora{args.lora_rank}-rmin{args.residual_r_min}-temp{args.temperature}")
